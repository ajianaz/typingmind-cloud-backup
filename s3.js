...
function openSyncModal() {
  ...
  modalPopup.innerHTML += `
	<div>
			<label for="s3-force-path-style" class="block text-sm font-medium text-gray-700 dark:text-gray-400">S3 Force Path Style</label>
			<select id="s3-force-path-style" name="s3-force-path-style" class="z-1 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700">
...
`
  ...
  const s3ForcePathStyleInput = document.getElementById('s3-force-path-style')
  ...
  const savedS3ForcePathStyle =
    localStorage.getItem('s3-force-path-style') === 'true'
  ...
  s3ForcePathStyleInput.value = savedS3ForcePathStyle ? 'true' : 'false'
  ...
  document
    ...
    .addEventListener('click', async function () {
      ...
      const s3ForcePathStyle =
        document.getElementById('s3-force-path-style').value === 'true'
      ...
      localStorage.setItem('s3-force-path-style', s3ForcePathStyle)
